...
////////////////////////////////////////////////////////////////////////
	...
	public static final		PatternGeneratorApp	INSTANCE	= new PatternGeneratorApp();
...
////////////////////////////////////////////////////////////////////////
		...
		throws AppException
	{
		...
		if (openFileChooser.showOpenDialog(mainWindow) == JFileChooser.APPROVE_OPTION)
		{
			...
			fileInfo = PatternDocument.FileInfo.of(openFileChooser.getSelectedFile(), documentKind);
		}
		return fileInfo;
	}
			...
			if (documentKind != null)
			{
				...
				if ((filenameDocumentKind != null) && (filenameDocumentKind != documentKind))
				{
					filename = StringUtils.removeSuffix(filename,
														filenameDocumentKind.getFilter().getSuffix(0)) + suffix;
					file = new File(file.getParentFile(), filename);
				}
				else
					file = Utils.appendSuffix(file, suffix);
			}
			fileInfo = PatternDocument.FileInfo.of(file, documentKind);
		}
		else
			fileInfo = null;
		return fileInfo;
	...
	private void updateConfiguration()
	{
		...
		config.setExportImageSequencePathname(Utils.getPathname(ExportImageSequenceDialog.getSelectedDirectory()));
	...
	private void openFiles(List<File> files)
	{
		for (int i = 0; i < files.size(); i++)
		{
			...
			try
			{
				openDocument(PatternDocument.FileInfo.of(files.get(i), null));
			}
			catch (AppException e)
			{
				if (i == files.size() - 1)
			...
			if (file == null)
			{
				...
				if (file != null)
				{
					file = new File(file.getParentFile(),
									document.getFilenameStem() + AppConstants.PNG_FILENAME_EXTENSION);
				}
			}

			file = chooseExportImage(file);
			if ((file != null) && confirmWriteFile(file, EXPORT_IMAGE_STR))
				TaskProgressDialog.showDialog(mainWindow, EXPORT_IMAGE_STR, new Task.ExportImage(document, file));
		}
	}
		...
		throws AppException
	{
		...
		if (document != null)
		{
			...
			if (image != null)
			{
				...
				params = ExportImageSequenceDialog.showDialog(mainWindow, image.getWidth(), image.getHeight(), params);
				if (params != null)
				{
		...
		throws AppException
	{
		...
		if ((document != null) && document.canExportAsSvg())
		{
			...
			if (file == null)
			{
				...
				{
					file = new File(file.getParentFile(),
									document.getFilenameStem() + AppConstants.SVG_FILENAME_EXTENSION);
				}
			}

			file = chooseExportSvg(file);
			if ((file != null) && confirmWriteFile(file, EXPORT_AS_SVG_STR))
				TaskProgressDialog.showDialog(mainWindow, EXPORT_AS_SVG_STR, new Task.ExportSvg(document, file));
		}
	}
...
////////////////////////////////////////////////////////////////////////
	...
	// CLASS: PAIRING OF DOCUMENT AND VIEW
